...
export default function BottomNav({ role }: { role: "parent" | "teacher" }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-2xl border-t border-gray-100 px-6 py-4 flex justify-between items-end z-[50] pb-8 shadow-[0_-10px_40px_rgba(0,0,0,0.02)]">
      {links.map((link) => {
          ...
          return (
            <Link key={link.href} href={link.href} className="relative mb-2">
              <motion.div 
                ...
                whileTap={{ scale: 0.95 }}
                className="w-16 h-16 bg-primary rounded-[24px] shadow-hpdt flex items-center justify-center text-white"
              >
                <link.icon size={28} fill="currentColor" />
              </motion.div>
